...
**Goal:** Constrain the CUDA workspace to CPython 3.12–3.14 and make uv resolve the GPU dependency graph only for Linux.
...
**Tech Stack:** Python 3.12–3.14, uv 0.12.3, PyTorch 2.11.0+cu129, Triton 3.6.0, NVIDIA CuTe DSL 4.6.1, Linux CUDA 12.9.1.

## Global Constraints

- Declare `requires-python = ">=3.12,<3.15"` in the root and every workspace member.
...
```bash
python3 -c 'import tomllib; root=tomllib.load(open("pyproject.toml", "rb")); member=tomllib.load(open("flash_attention_triton/pyproject.toml", "rb")); assert root["project"]["requires-python"] == ">=3.12,<3.15"; assert member["project"]["requires-python"] == ">=3.12,<3.15"; assert root["tool"]["uv"]["environments"] == ["sys_platform == '\''linux'\''"]; assert root["tool"]["uv"]["sources"]["torch"] == [{"index": "pytorch-cu129", "marker": "sys_platform == '\''linux'\''"}]'
```

Expected: FAIL because the Python upper bound and Linux-only uv/source declarations are absent.

- [ ] **Step 2: Apply the minimal metadata change**

Set both `requires-python` declarations to `>=3.12,<3.15`. Add the following to `[tool.uv]` and narrow the existing Torch source marker:
...
Record CPython 3.12–3.14 and Linux-only support in the research note and the original CUDA implementation plan. Remove claims that cu129 resolves Windows wheels; retain Windows only where documenting it as unsupported.
...
Expected: FAIL because project metadata changed while `uv.lock` still declares `>=3.12` and the old platform split.
...
- Produces: A consistent lockfile that installs Torch 2.11.0+cu129 only for Linux and excludes Python 3.15 from the project contract.
...
```bash
uv lock --check
python3 -c 'import tomllib; lock=tomllib.load(open("uv.lock", "rb")); assert {part.strip() for part in lock["requires-python"].split(",")} == {">=3.12", "<3.15"}'
...
```bash
uv sync --locked
uv run --locked python -c 'import cutlass, cutlass.cute, torch, triton; assert cutlass.__version__ == "4.6.1"; assert torch.__version__ == "2.11.0+cu129"; assert torch.version.cuda == "12.9"; print(cutlass.__version__, torch.__version__, triton.__version__)'
```

Expected: the environment is synchronized and all imports/assertions succeed.

- [ ] **Step 4: Verify unsupported contracts are rejected**

```bash
uv sync --locked --dry-run --python-platform x86_64-pc-windows-msvc
uv pip install --dry-run --reinstall --python-version 3.15 'nvidia-cutlass-dsl==4.6.1'
```

Expected: both commands fail. The first must report that the lock/project environment does not support Windows; the second must report the missing `cp315` native CuTe wheel.
...
```bash
git add pyproject.toml flash_attention_triton/pyproject.toml uv.lock docs/research/2026-08-11-gpu-kernel-harness.md docs/superpowers/plans/2026-08-11-cuda-12-9-1-environment.md docs/superpowers/plans/2026-08-12-linux-python-contract.md
